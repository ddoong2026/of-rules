...
function TeacherBoard({state,operate}) {
  ...
  return <><div className={styles.notice}><strong>준비 상태: 시각 자료 검수 대기</strong>{readiness().map(t=><p key={t}>{t}</p>)}</div><section className={styles.panel}><h2>수업 진행</h2><div className={styles.row}><button aria-pressed={!!state.locked} onClick={()=>operate({type:'lock',locked:!state.locked})}>{state.locked?'학생 조작 다시 시작':'전체 학생 조작 멈춤'}</button><button onClick={()=>operate({type:'reveal'})}>지정한 모둠·주제 공개</button><button onClick={()=>operate({type:'publish'})}>관찰 메모 전체 공개</button><button onClick={()=>operate({type:'assignFeedback'})}>피드백 대상 배정 / 재배정</button><button className={styles.primary} onClick={()=>operate({type:'approve'})}>피드백한 맵 열기(전체 승인)</button></div><div className={styles.row}><select aria-label="이동할 활동" value={activity} onChange={e=>setActivity(e.target.value)}>{ACTIVITIES.map(a=><option key={a} value={a}>{a==='3D'?'2D 체험':a}</option>)}</select><button onClick={()=>operate({type:'move',activity,targets:students.map(s=>s.id)})}>전체 학생 보내기</button><button disabled={!targets.length} onClick={()=>operate({type:'move',activity,targets})}>선택한 {targets.length}명 보내기</button></div><p className={styles.muted}>현재 입력을 보존하고 지정한 활동으로 이동합니다. 미체험 단서는 보충으로 제공하며 퀘스트 완료로 처리하지 않습니다.</p></section>
    <details className={styles.panel}><summary>전체 관찰 메모 · 교사가 위치 옮기기</summary><div className={styles.teacherObservation}><ObservationBoard state={{published:state.published,notes:students.flatMap(s=>s.notes.map(n=>({...n,author:s.name,authorId:s.id})))}} draft={{notes:[]}} onMoveNote={(note,patch)=>operate({type:'moveNote',owner:note.authorId,noteId:note.id,...patch})}/></div></details><section className={styles.panel}><h2>학생별 진행</h2><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>선택</th><th>학생</th><th>개인 주제</th><th>활동</th><th>명령 수신</th><th>저장</th><th>연결</th><th>체험</th><th>해금</th><th>기록</th></tr></thead><tbody>{students.map(s=><tr key={s.id}><td><input type="checkbox" aria-label={`${s.name} 선택`} checked={targets.includes(s.id)} onChange={e=>setTargets(e.target.checked?[...targets,s.id]:targets.filter(id=>id!==s.id))}/></td><td>{s.name}</td><td>{s.path} {pathById(s.path).title}</td><td>{s.activity==='3D'?'2D 체험':s.activity}</td><td>{s.command?.sequence>s.ack?'미수신':'적용됨'}</td><td>{s.saveState || '미작성'}</td><td>{s.lastSeen && now-Date.parse(s.lastSeen)<30000?'연결됨':'응답 대기'}</td><td>{s.attempts[s.path]?.complete?'완료':s.attempts[s.path]?.interrupted?'중단':'미완료'}</td><td>{s.grants.length}개</td><td><button onClick={()=>setInspect(s.id)}>열람</button></td></tr>)}</tbody></table></div></section>
...
function StudentWorkspace({state,draft,change,operate,preview}) {
  ...
  return <><div className={styles.steps}>{ACTIVITIES.map((a,i)=><span key={a} className={a===activity?styles.active:''}>{String(i+1).padStart(2,'0')} {a==='3D'?'2D 체험':a}</span>)}</div>
...
function Experience({path,research,attempt,operate,diary,onDiaryChange,versions}) {
  ...
  const diaryPage=<section className={styles.diary} aria-label="탐험 일기">
    ...
    {path.questions.map(q=><div key={q.id}><strong>{q.prompt}</strong><p>{q.options[record.answers[q.id]] || 'NPC와 대화하고 퀴즈를 풀어 기록해요.'}</p>{step>=5&&<label>일기 문장 확인<select value={record.answers[q.id] ?? ''} onChange={e=>{const answers={...record.answers};if(e.target.value==='')delete answers[q.id];else answers[q.id]=Number(e.target.value);update({...record,answers});}}><option value="">문장 선택</option>{q.options.map((o,i)=><option key={o} value={i}>{o}</option>)}</select>{record.answers[q.id]!==q.answer&&<small className={styles.clue}>{q.hint}</small>}</label>}</div>)}
    {step>=5&&<><p>{story.reflection}</p><label>그때의 나였다면 느꼈을 감정과 생각<textarea value={record.text} placeholder="배운 유물과 생활을 떠올리며 나의 하루를 써 보세요." onChange={e=>update({...record,text:e.target.value})}/></label></>}
  ...
  const actions=<>
    {question&&<fieldset><legend>유물과 생활 퀴즈 · 일기 문장 고르기</legend><p>{question.prompt}</p>{question.options.map((option,i)=><label key={option}><input type="radio" name={question.id} checked={record.answers[question.id]===i} onChange={()=>update({...record,answers:{...record.answers,[question.id]:i}})}/>{option}</label>)}{record.answers[question.id]!==undefined&&<p role="status" className={styles.clue}>{record.answers[question.id]===question.answer?'정답이에요! 이 문장이 일기에 기록되었어요.':`단서를 다시 읽고 골라 보세요: ${question.hint}`}</p>}</fieldset>}
    {step>=5&&diaryPage}
