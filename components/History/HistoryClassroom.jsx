...
function Experience({path,research,attempt,operate,preview}) {
  const [alternative,setAlternative]=useState(false),[question,setQuestion]=useState(0),[choice,setChoice]=useState(''),[visitedStep,setVisitedStep]=useState(-1);
  const step=attempt?.checkpoint || 0;
  const story=STORIES[path.id],lines=storyLines(path);
  return <section className={styles.panel}><p className={styles.muted}>{preview?'2D 탐험 · 학습용 재구성':'학습을 위한 재구성'} · 교과서 {path.pages}쪽</p><header><span className={styles.badge}>나의 이야기 · {story.role}</span><h3>{story.title}</h3><p className={styles.muted}>{STORY_NOTICE}</p></header><div className={styles.row}><button onClick={()=>setAlternative(v=>!v)}>{alternative?'2D 게임 보기':'텍스트 대체 흐름 사용'}</button></div>{!alternative && <Game2D key={path.id} path={path} research={research} step={step} onInteract={()=>setVisitedStep(step)}/>}<div className={styles.row}><span className={styles.badge}>{Math.min(step+1,STEPS.length)} / {STEPS.length}</span><h3>{STEPS[step] || '체험 완료'}</h3></div>{step===3 && <p className={styles.notice}>다른 시대 또는 상황의 장면을 살펴봅니다. 같은 사람이 수백 년을 산다는 뜻이 아니에요.</p>}<p className={styles.quote}>{[1,3,4].includes(step)?<strong>{lines[step]}</strong>:lines[step] || '기록한 단서를 바탕으로 하루를 써 볼까요?'}</p>{[1,3,4].includes(step)&&<p className={styles.clue}>교과서 단서: {path.clues[step===1?0:step===3?1:question]}</p>}{step===4 && <div className={styles.row}>{['생활은 어떠했나요?','무엇이 달라졌나요?'].map((q,i)=><button key={q} onClick={()=>setQuestion(i)}>{q}</button>)}</div>}{step===5 && <label>단서와 연결되는 생각<select value={choice} onChange={e=>setChoice(e.target.value)}><option value="">선택해 주세요</option>{path.questions[0].options.map(o=><option key={o}>{o}</option>)}</select>{choice && choice!==path.questions[0].options[0] && <small>다시 읽을 단서: {path.clues[0]}</small>}</label>}{step<STEPS.length && <button className={styles.primary} disabled={(step===5&&!choice)||(!alternative&&visitedStep!==step)} onClick={()=>operate({type:'step',path:path.id,step,mode:alternative?'alternative':'2D'})}>{step===0?'주변 관찰 마치기':step===6?'체험 기록 마치기':'읽고 다음으로 →'}</button>}{step===STEPS.length && path.group===3 && <details><summary>＋ 추가 문제</summary><p>삼국은 왜 한강 유역을 차지하려 했을까요? 교과서 33~35쪽 지도의 위치와 교류의 길을 연결해 이야기해 봅시다.</p><p>가야는 철과 교류를 바탕으로 성장했습니다. 삼국과 어떤 점이 같고 다른가요?</p></details>}</section>;
}
