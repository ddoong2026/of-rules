...
export default function HistoryGame2D({path,research=[],step,onInteract}) {
  ...
  function tick(time) {
    ...
    if(goal){
      ...
      if(distance<4){stop();if(goal.interact&&step<7){setDialog(step);onInteract();}return;}
  ...
  function moveTo(point,interact=false){stop();setDialog(-1);destination.current={...point,interact};frame.current=requestAnimationFrame(tick);}
  function clickMap(e){
