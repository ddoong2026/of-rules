...
export function createLesson(roster) {
  ...
  return { version: VERSION, published:false, locked:false, revealed:false, sequence:0, activity:'준비', commands:[], operations:[], cards:{}, feedback:{}, students:Object.fromEntries(roster.map((s,i)=>[s.id,{id:s.id,name:s.name,path:PATHS[i].id,group:PATHS[i].group,activity:'준비',ack:0,revision:0,notes:[],research:{},diary:{answers:{},text:'',versions:[]},feedbackDraft:{reaction:'',text:''},attempts:{},grants:[],lastSeen:null}])) };
}
...
export function applyOperation(original, actor, teacher, op, preview=false) {
  ...
  if(op.type==='lock' || op.type==='moveNote' || op.type==='publish' || op.type==='reveal' || op.type==='move' || op.type==='assignFeedback' || op.type==='approve') {
    ...
    if(op.type==='ack') {
      ...
      for(const [id,r] of Object.entries(d.research || {})) {
        ...
        if(changed) state.cards[id]={id,artifact:r.artifact,group:s.group,owner:old?.owner || actor,helpers:[...new Set([...(old?.helpers || []),actor])],...fields,revision:(old?.revision || 0)+1,status:'초안',versions:old?.versions || []};
...
export function studentView(state,id) {
  ...
  return {version:state.version,published:state.published,locked:!!state.locked,revealed:state.revealed,activity:state.activity,me:s,cards:Object.values(state.cards).filter(c=>c.group===s.group),notes:Object.values(state.students).flatMap(x=>state.published || x.id===id?x.notes.map(n=>({...n,author:x.name,authorId:x.id})):[]),diaries:Object.values(state.students).filter(x=>x.diary.versions.length).map(x=>({id:x.id,name:x.name,path:x.path,versions:x.diary.versions})),feedback:Object.values(state.feedback).filter(f=>f.author===id || f.recipient===id)};
}
