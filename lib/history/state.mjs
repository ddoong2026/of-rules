...
export function applyOperation(original, actor, teacher, op, preview=false) {
  ...
  if(op.type==='publish' || op.type==='reveal' || op.type==='move' || op.type==='assignFeedback' || op.type==='approve') {
    ...
    if(op.type==='ack') {
      ...
      s.notes=d.notes.map(n=>({id:text(n.id),image:n.image,x:n.x,y:n.y,kind:n.kind,text:text(n.text),...(n.noteX!==undefined?{noteX:n.noteX}:{}),...(n.noteY!==undefined?{noteY:n.noteY}:{})}));
      const answers={};
