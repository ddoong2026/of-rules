  ...
  if(op.type==='noteVisibility' || op.type==='lock' || op.type==='moveNote' || op.type==='publish' || op.type==='reveal' || op.type==='move' || op.type==='assignFeedback' || op.type==='approve') {
    ...
    if(op.type==='ack') {
      ...
      for(const [id,r] of Object.entries(d.research || {})) {
        ...
        if(old ? old.artifact!==r.artifact : !PATHS.filter(p=>p.group===s.group).some(p=>p.artifacts.includes(r.artifact))) fail('조사 카드가 올바르지 않습니다.');
        ...
        if(changed) state.cards[id]={id,artifact:r.artifact,group:old?.group??s.group,owner:old?.owner || actor,helpers:[...new Set([...(old?.helpers || []),actor])],...fields,revision:(old?.revision || 0)+1,status:'초안',versions:old?.versions || []};
