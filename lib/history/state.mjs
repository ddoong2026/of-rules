...
export function createLesson(roster) {
  ...
  return { version: VERSION, published:false, liveNotes:false, publishedNotes:[], locked:false, revealed:false, scoreVisible:false, sequence:0, activity:'준비', commands:[], operations:[], cards:{}, chat:[], students:Object.fromEntries(roster.map((s,i)=>[s.id,{id:s.id,name:s.name,path:PATHS[i].id,group:PATHS[i].group,activity:'준비',ack:0,revision:0,notes:[],research:{},diary:{answers:{},text:'',versions:[]},attempts:{},grants:[],reviewedGroups:[],reviewQuizzes:{},lastSeen:null}])) };
}
...
export function applyOperation(original, actor, teacher, op) {
  ...
  if(op.type==='noteVisibility' || op.type==='lock' || op.type==='moveNote' || op.type==='publish' || op.type==='reveal' || op.type==='move' || op.type==='scoreVisibility') {
    if(!teacher) fail('교사만 실행할 수 있습니다.');
    if(op.type==='lock') {
    ...
    if(op.type==='ack') {
      ...
      if(path!==s.path && !s.grants.includes(path)) fail('체험할 수 있는 맵이 아닙니다.');
      ...
      const question=reviewQuestions(group).find(q=>q.id===op.questionId);
      if(!question||!Number.isInteger(op.choice)||op.choice<0||op.choice>=question.options.length) fail('복습 퀴즈의 문제와 답을 확인해 주세요.');
      s.reviewQuizzes ||= {};
      const record=s.reviewQuizzes[group] ||= {};
      ...
      if(status.sameGroup && !status.ownDone) fail('내 맵을 먼저 완료해야 같은 모둠의 다른 맵을 체험할 수 있어요.');
      ...
      if(status.quizScore<3) fail('복습 퀴즈를 3문제 이상 맞혀야 체험할 수 있어요.');
      s.grants.push(op.path);
    } else if(op.type==='enterExperience') {
      if(!['3D','일기','정리'].includes(s.activity)||(op.path===s.path?s.activity!=='3D':!s.grants.includes(op.path)))fail('체험할 수 있는 맵만 선택할 수 있습니다.');
...
export function reviewQuestions(group) {
  return PATHS.filter(p=>p.group===Number(group)).flatMap(p=>p.questions.map((q,i)=>{
...
export function reviewQuizScore(student,group) {
  return reviewQuestions(group).filter(q=>student.reviewQuizzes?.[group]?.[q.id]?.choice===q.answer).length;
}
export function reviewQuizComplete(student,group) {
  return reviewQuestions(group).every(q=>student.reviewQuizzes?.[group]?.[q.id]?.choice===q.answer);
}
export function reviewQuizRecord(student,group) {
  return student.reviewQuizzes?.[group]||{};
}
// The three-correct-answer gate always reuses the same review quiz bank a group already has;
// same-group unlocks only need the student's own map finished, cross-group unlocks additionally
// need the whole home group finished and that other group's research cards reviewed first.
export function unlockStatus(student,targetPath) {
  ...
  const ownDone=!!student.attempts[student.path]?.complete;
  const groupDone=groupComplete(student,student.group);
  const reviewed=sameGroup||!!(student.reviewedGroups||[]).includes(target.group);
  const quizScore=reviewQuizScore(student,target.group);
  const quizNeeded=3;
  const ok=!already&&(sameGroup?ownDone:groupDone&&reviewed)&&quizScore>=quizNeeded;
  return {path:targetPath,group:target.group,sameGroup,already,ownDone,groupDone,reviewed,quizScore,quizNeeded,ok};
}
