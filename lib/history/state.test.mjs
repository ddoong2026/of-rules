...
import {createLesson,applyOperation,studentView,groupComplete,unlockStatus,chainQuizProgress,groupScore,groupScores} from './state.mjs';
...
test('the chain quiz reuses the finished map\'s own two questions, then adds one from the map before it',()=>{
  let state=createLesson(roster);
  const id='s0';
  state.students[id].activity='3D';
  const own=state.students[id].path;
  const ownQuestions=PATHS.find(p=>p.id===own).questions;
  assert.deepEqual(chainQuizProgress(state.students[id]).questions,[]);
  state=completePath(state,id,own);
  let progress=chainQuizProgress(state.students[id]);
  assert.equal(progress.current,own);
  assert.equal(progress.needed,2);
  assert.deepEqual(progress.questions.map(q=>q.id),ownQuestions.map(q=>q.id));
  const home=state.students[id].group;
  const sibling=PATHS.find(p=>p.group===home&&p.id!==own).id;
  for(const q of progress.questions)state=op(state,id,'answerReviewQuiz',{questionId:q.id,choice:q.answer});
  assert.equal(chainQuizProgress(state.students[id]).passed,true);
  state=op(state,id,'unlockMap',{path:sibling});
  state=completePath(state,id,sibling);
  progress=chainQuizProgress(state.students[id]);
  assert.equal(progress.current,sibling);
  assert.equal(progress.needed,3);
  const siblingQuestions=PATHS.find(p=>p.id===sibling).questions;
  assert.deepEqual(progress.questions.map(q=>q.id),[...siblingQuestions.map(q=>q.id),ownQuestions[0].id]);
});

test('same-group map unlock requires the chain quiz passed, and only one unlocked map may be pending at a time',()=>{
  let state=createLesson(roster);
  const id='s0';
  state.students[id].activity='3D';
  const home=state.students[id].group,own=state.students[id].path;
  const sibling=PATHS.find(p=>p.group===home&&p.id!==own).id;
  assert.throws(()=>op(state,id,'unlockMap',{path:sibling}),/문제를 모두 맞혀야/);
  state=completePath(state,id,own);
  assert.equal(state.students[id].attempts[own].complete,true);
  const questions=chainQuizProgress(state.students[id]).questions;
  state=op(state,id,'answerReviewQuiz',{questionId:questions[0].id,choice:questions[0].answer});
  assert.throws(()=>op(state,id,'unlockMap',{path:sibling}),/문제를 모두 맞혀야/);
  state=op(state,id,'answerReviewQuiz',{questionId:questions[1].id,choice:questions[1].answer});
  assert.equal(chainQuizProgress(state.students[id]).passed,true);
  state=op(state,id,'unlockMap',{path:sibling});
  ...
  const secondSibling=PATHS.find(p=>p.group===home&&p.id!==own&&p.id!==sibling).id;
  assert.throws(()=>op(state,id,'unlockMap',{path:secondSibling}),/먼저 체험/);
});
...
test('cross-group map unlock requires the whole home group finished, that group reviewed, and the chain quiz passed',()=>{
  let state=createLesson(roster);
  const id='s0';
  state.students[id].activity='3D';
  const home=state.students[id].group,own=state.students[id].path;
  ...
  for(const p of PATHS.filter(x=>x.group===home&&x.id!==own)) {
    for(const q of chainQuizProgress(state.students[id]).questions)state=op(state,id,'answerReviewQuiz',{questionId:q.id,choice:q.answer});
...
test('teacher free mode bypasses map unlocking, research-group gating, and quiz gates entirely',()=>{
  let state=createLesson(roster);
  const id='s0';
  state.students[id].activity='3D';
  assert.throws(()=>op(state,id,'freeMode',{enabled:true}),/교사/);
  assert.throws(()=>op(state,'t','freeMode',{enabled:'yes'},true),/모드 상태/);
  assert.throws(()=>op(state,id,'step',{path:'4-D',step:0}),/체험할 수 있는 맵이 아닙니다/);
  assert.equal(studentView(state,id).canHelpResearch,false);
  state=op(state,'t','freeMode',{enabled:true},true);
  assert.equal(studentView(state,id).freeMode,true);
  assert.equal(studentView(state,id).canHelpResearch,true);
  state=op(state,id,'step',{path:'4-D',step:0,mode:'2D'});
  assert.equal(state.students[id].attempts['4-D'].checkpoint,1);
  state=op(state,id,'enterExperience',{path:'2-C'});
  assert.equal(state.students[id].activePath,'2-C');
  assert.throws(()=>op(state,id,'enterExperience',{path:'not-a-path'}),/존재하지/);
  state=op(state,id,'answerReviewQuiz',{path:'3-A',questionId:PATHS.find(p=>p.id==='3-A').questions[0].id,choice:0});
  assert.equal(state.students[id].reviewQuizzes['3-A'][PATHS.find(p=>p.id==='3-A').questions[0].id].choice,0);
  state=op(state,id,'unlockMap',{path:'1-D'});
  assert.ok(state.students[id].grants.includes('1-D'));
  state=op(state,'t','freeMode',{enabled:false},true);
  assert.equal(studentView(state,id).freeMode,false);
  assert.throws(()=>op(state,id,'step',{path:'2-B',step:0}),/체험할 수 있는 맵이 아닙니다/);
});
